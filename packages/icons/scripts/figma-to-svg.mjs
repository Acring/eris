...
const skipComponents = options.skipList
  ? options.skipList.split(',').map((name) => name.trim())
  : [];
console.log(`Skip list: ${skipComponents.length > 0 ? skipComponents.join(', ') : 'None'}`);
...
if (!fileId) {
  try {
    fileId = FIGMA_FILE_URL.match(/design\/([a-z0-9]+)\//i)[1];
  } catch (e) {
  ...
  .then(({ data }) => {
    ...
    function check(c) {
      if (c.type === 'COMPONENT') {
        ...
        if (skipComponents.includes(name)) {
